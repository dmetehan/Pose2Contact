...
def get_baseline_predictors(all_labels):
    all_strategies = ["most_frequent", "prior", "stratified", "uniform"]
    all_clfs = {key: {strategy: DummyClassifier(strategy=strategy) for strategy in all_strategies} for key in all_labels}
    for key in all_clfs:
